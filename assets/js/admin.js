...
(function () {
  ...
  function comprobar() {
    var visto = null;
    try { visto = sessionStorage.getItem("mf.admin"); } catch (e) { /* nada */ }
    ...
    var comprobado = SB.hasSession()
      ? SB.getSession().then(function (s) {
          if (!s) return false;
          return SB.rpc("admin_is").then(function (si) { return si === true; }).catch(function () { return false; });
        })
      : Promise.resolve(false);
    return comprobado.then(function (si) {
      try { sessionStorage.setItem("mf.admin", si ? "1" : "0"); } catch (e) { /* nada */ }
      if (si) revelar();
      return si;
    });
  }
